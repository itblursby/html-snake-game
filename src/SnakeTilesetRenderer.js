...
export const headTiles = [[8,80], [40,80], [72,80], [104, 80]];

// tail tiles, reference by forward direction (against tail direction)
export const tailTiles = [[8,208], [40,208], [72,208], [104, 208]];
...
export function renderSnake(coords) {
    const toRender = [];

    const headPoint = coords[coords.length-1];
    const drawHeadPoint = [headPoint[0] * 16 - 8, headPoint[1]*16-16];
    ...
    const drawTailPoint = [tailPoint[0] * 16 - 8, tailPoint[1]*16-16];
    ...
    for (let i = 1; i < coords.length -1; i++) {
        ...
        const drawPoint = [point[0] * 16 - 8, point[1]*16-16];
